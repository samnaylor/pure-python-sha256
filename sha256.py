# Implemented from the pseudocode at https://en.wikipedia.org/wiki/SHA-2

from collections.abc import Iterator

__all__ = ["sha224", "sha256", "sha512"]


SHA224_H = (
    0xC1059ED8,
    0x367CD507,
    0x3070DD17,
    0xF70E5939,
    0xFFC00B31,
    0x68581511,
    0x64F98FA7,
    0xBEFA4FA4,
)

SHA224_K = [
    0x428A2F98,
    0x71374491,
    0xB5C0FBCF,
    0xE9B5DBA5,
    0x3956C25B,
    0x59F111F1,
    0x923F82A4,
    0xAB1C5ED5,
    0xD807AA98,
    0x12835B01,
    0x243185BE,
    0x550C7DC3,
    0x72BE5D74,
    0x80DEB1FE,
    0x9BDC06A7,
    0xC19BF174,
    0xE49B69C1,
    0xEFBE4786,
    0x0FC19DC6,
    0x240CA1CC,
    0x2DE92C6F,
    0x4A7484AA,
    0x5CB0A9DC,
    0x76F988DA,
    0x983E5152,
    0xA831C66D,
    0xB00327C8,
    0xBF597FC7,
    0xC6E00BF3,
    0xD5A79147,
    0x06CA6351,
    0x14292967,
    0x27B70A85,
    0x2E1B2138,
    0x4D2C6DFC,
    0x53380D13,
    0x650A7354,
    0x766A0ABB,
    0x81C2C92E,
    0x92722C85,
    0xA2BFE8A1,
    0xA81A664B,
    0xC24B8B70,
    0xC76C51A3,
    0xD192E819,
    0xD6990624,
    0xF40E3585,
    0x106AA070,
    0x19A4C116,
    0x1E376C08,
    0x2748774C,
    0x34B0BCB5,
    0x391C0CB3,
    0x4ED8AA4A,
    0x5B9CCA4F,
    0x682E6FF3,
    0x748F82EE,
    0x78A5636F,
    0x84C87814,
    0x8CC70208,
    0x90BEFFFA,
    0xA4506CEB,
    0xBEF9A3F7,
    0xC67178F2,
]

SHA224_BITSIZE = 32
SHA224_BITMASK = 0xFFFFFFFF


SHA256_H = (0x6A09E667, 0xBB67AE85, 0x3C6EF372, 0xA54FF53A, 0x510E527F, 0x9B05688C, 0x1F83D9AB, 0x5BE0CD19)

SHA256_K = [
    0x428A2F98,
    0x71374491,
    0xB5C0FBCF,
    0xE9B5DBA5,
    0x3956C25B,
    0x59F111F1,
    0x923F82A4,
    0xAB1C5ED5,
    0xD807AA98,
    0x12835B01,
    0x243185BE,
    0x550C7DC3,
    0x72BE5D74,
    0x80DEB1FE,
    0x9BDC06A7,
    0xC19BF174,
    0xE49B69C1,
    0xEFBE4786,
    0x0FC19DC6,
    0x240CA1CC,
    0x2DE92C6F,
    0x4A7484AA,
    0x5CB0A9DC,
    0x76F988DA,
    0x983E5152,
    0xA831C66D,
    0xB00327C8,
    0xBF597FC7,
    0xC6E00BF3,
    0xD5A79147,
    0x06CA6351,
    0x14292967,
    0x27B70A85,
    0x2E1B2138,
    0x4D2C6DFC,
    0x53380D13,
    0x650A7354,
    0x766A0ABB,
    0x81C2C92E,
    0x92722C85,
    0xA2BFE8A1,
    0xA81A664B,
    0xC24B8B70,
    0xC76C51A3,
    0xD192E819,
    0xD6990624,
    0xF40E3585,
    0x106AA070,
    0x19A4C116,
    0x1E376C08,
    0x2748774C,
    0x34B0BCB5,
    0x391C0CB3,
    0x4ED8AA4A,
    0x5B9CCA4F,
    0x682E6FF3,
    0x748F82EE,
    0x78A5636F,
    0x84C87814,
    0x8CC70208,
    0x90BEFFFA,
    0xA4506CEB,
    0xBEF9A3F7,
    0xC67178F2,
]

SHA256_BITSIZE = 32
SHA256_BITMASK = 0xFFFFFFFF


SHA512_H = (
    0x6A09E667F3BCC908,
    0xBB67AE8584CAA73B,
    0x3C6EF372FE94F82B,
    0xA54FF53A5F1D36F1,
    0x510E527FADE682D1,
    0x9B05688C2B3E6C1F,
    0x1F83D9ABFB41BD6B,
    0x5BE0CD19137E2179,
)

SHA512_K = [
    0x428A2F98D728AE22,
    0x7137449123EF65CD,
    0xB5C0FBCFEC4D3B2F,
    0xE9B5DBA58189DBBC,
    0x3956C25BF348B538,
    0x59F111F1B605D019,
    0x923F82A4AF194F9B,
    0xAB1C5ED5DA6D8118,
    0xD807AA98A3030242,
    0x12835B0145706FBE,
    0x243185BE4EE4B28C,
    0x550C7DC3D5FFB4E2,
    0x72BE5D74F27B896F,
    0x80DEB1FE3B1696B1,
    0x9BDC06A725C71235,
    0xC19BF174CF692694,
    0xE49B69C19EF14AD2,
    0xEFBE4786384F25E3,
    0x0FC19DC68B8CD5B5,
    0x240CA1CC77AC9C65,
    0x2DE92C6F592B0275,
    0x4A7484AA6EA6E483,
    0x5CB0A9DCBD41FBD4,
    0x76F988DA831153B5,
    0x983E5152EE66DFAB,
    0xA831C66D2DB43210,
    0xB00327C898FB213F,
    0xBF597FC7BEEF0EE4,
    0xC6E00BF33DA88FC2,
    0xD5A79147930AA725,
    0x06CA6351E003826F,
    0x142929670A0E6E70,
    0x27B70A8546D22FFC,
    0x2E1B21385C26C926,
    0x4D2C6DFC5AC42AED,
    0x53380D139D95B3DF,
    0x650A73548BAF63DE,
    0x766A0ABB3C77B2A8,
    0x81C2C92E47EDAEE6,
    0x92722C851482353B,
    0xA2BFE8A14CF10364,
    0xA81A664BBC423001,
    0xC24B8B70D0F89791,
    0xC76C51A30654BE30,
    0xD192E819D6EF5218,
    0xD69906245565A910,
    0xF40E35855771202A,
    0x106AA07032BBD1B8,
    0x19A4C116B8D2D0C8,
    0x1E376C085141AB53,
    0x2748774CDF8EEB99,
    0x34B0BCB5E19B48A8,
    0x391C0CB3C5C95A63,
    0x4ED8AA4AE3418ACB,
    0x5B9CCA4F7763E373,
    0x682E6FF3D6B2B8A3,
    0x748F82EE5DEFB2FC,
    0x78A5636F43172F60,
    0x84C87814A1F0AB72,
    0x8CC702081A6439EC,
    0x90BEFFFA23631E28,
    0xA4506CEBDE82BDE9,
    0xBEF9A3F7B2C67915,
    0xC67178F2E372532B,
    0xCA273ECEEA26619C,
    0xD186B8C721C0C207,
    0xEADA7DD6CDE0EB1E,
    0xF57D4F7FEE6ED178,
    0x06F067AA72176FBA,
    0x0A637DC5A2C898A6,
    0x113F9804BEF90DAE,
    0x1B710B35131C471B,
    0x28DB77F523047D84,
    0x32CAAB7B40C72493,
    0x3C9EBE0A15C9BEBC,
    0x431D67C49C100D4C,
    0x4CC5D4BECB3E42B6,
    0x597F299CFC657E2A,
    0x5FCB6FAB3AD6FAEC,
    0x6C44198C4A475817,
]

SHA512_BITSIZE = 64
SHA512_BITMASK = 0xFFFFFFFFFFFFFFFF


def _rr(x: int, n: int, bit_size: int, mask: int) -> int:
    return ((x >> n) | (x << (bit_size - n))) & mask


def __int_from_bytes(message: bytes) -> int:
    return int.from_bytes(message, "big")


def __pad(message: bytes, pad_size: int) -> int:
    ln = len(message) << 3
    pd = pad_size // 8
    return ((__int_from_bytes(message) << 1) | 1) << ((pad_size + pd) - ((1 + pd + ln) % pad_size)) | ln


def __isolate(value: int, nbits: int, start_bit: int) -> int:
    return (value & (((1 << nbits) - 1) << start_bit)) >> (start_bit)


def __chunk(padded: int, chunk_size: int) -> Iterator[int]:
    for i in range(((padded.bit_length() + 2) // chunk_size) - 1, -1, -1):
        yield __isolate(padded, chunk_size, (i * chunk_size))


def sha224(message: bytes | str) -> str:
    if isinstance(message, str):
        message = message.encode("utf-8")

    h0, h1, h2, h3, h4, h5, h6, h7 = SHA224_H
    k = SHA224_K
    bit_size = SHA224_BITSIZE
    mask = SHA224_BITMASK

    for chunk in __chunk(__pad(message, 512), 512):
        w = [0] * 64

        for i in range(15, -1, -1):
            w[15 - i] = __isolate(chunk, 32, (i * 32))

        for i in range(16, 64):
            s0 = _rr(w[i - 15], 7, bit_size, mask) ^ _rr(w[i - 15], 18, bit_size, mask) ^ (w[i - 15] >> 3)
            s1 = _rr(w[i - 2], 17, bit_size, mask) ^ _rr(w[i - 2], 19, bit_size, mask) ^ (w[i - 2] >> 10)
            w[i] = (w[i - 16] + s0 + w[i - 7] + s1) & 0xFFFFFFFF

        a, b, c, d, e, f, g, h = h0, h1, h2, h3, h4, h5, h6, h7

        for i in range(64):
            ts1 = _rr(e, 6, bit_size, mask) ^ _rr(e, 11, bit_size, mask) ^ _rr(e, 25, bit_size, mask)
            ch = (e & f) ^ ((~e) & g)
            t1 = (h + ts1 + ch + k[i] + w[i]) & 0xFFFFFFFF
            ts0 = _rr(a, 2, bit_size, mask) ^ _rr(a, 13, bit_size, mask) ^ _rr(a, 22, bit_size, mask)
            ma = (a & b) ^ (a & c) ^ (b & c)
            t2 = (ts0 + ma) & 0xFFFFFFFF

            h, g, f, e, d, c, b, a = g, f, e, (d + t1) & 0xFFFFFFFF, c, b, a, (t1 + t2) & 0xFFFFFFFF

        h0 = (h0 + a) & 0xFFFFFFFF
        h1 = (h1 + b) & 0xFFFFFFFF
        h2 = (h2 + c) & 0xFFFFFFFF
        h3 = (h3 + d) & 0xFFFFFFFF
        h4 = (h4 + e) & 0xFFFFFFFF
        h5 = (h5 + f) & 0xFFFFFFFF
        h6 = (h6 + g) & 0xFFFFFFFF
        h7 = (h7 + h) & 0xFFFFFFFF

    digest = 0

    for i, v in enumerate([h0, h1, h2, h3, h4, h5, h6]):
        digest = digest + (v << ((6 - i) * 32))

    return f"{digest:x}"


def sha256(message: bytes | str) -> str:
    if isinstance(message, str):
        message = message.encode("utf-8")

    h0, h1, h2, h3, h4, h5, h6, h7 = SHA256_H
    k = SHA256_K
    bit_size = SHA256_BITSIZE
    mask = SHA256_BITMASK

    for chunk in __chunk(__pad(message, 512), 512):
        w = [0] * 64

        for i in range(15, -1, -1):
            w[15 - i] = __isolate(chunk, 32, (i * 32))

        for i in range(16, 64):
            s0 = _rr(w[i - 15], 7, bit_size, mask) ^ _rr(w[i - 15], 18, bit_size, mask) ^ (w[i - 15] >> 3)
            s1 = _rr(w[i - 2], 17, bit_size, mask) ^ _rr(w[i - 2], 19, bit_size, mask) ^ (w[i - 2] >> 10)
            w[i] = (w[i - 16] + s0 + w[i - 7] + s1) & 0xFFFFFFFF

        a, b, c, d, e, f, g, h = h0, h1, h2, h3, h4, h5, h6, h7

        for i in range(64):
            ts1 = _rr(e, 6, bit_size, mask) ^ _rr(e, 11, bit_size, mask) ^ _rr(e, 25, bit_size, mask)
            ch = (e & f) ^ ((~e) & g)
            t1 = (h + ts1 + ch + k[i] + w[i]) & 0xFFFFFFFF
            ts0 = _rr(a, 2, bit_size, mask) ^ _rr(a, 13, bit_size, mask) ^ _rr(a, 22, bit_size, mask)
            ma = (a & b) ^ (a & c) ^ (b & c)
            t2 = (ts0 + ma) & 0xFFFFFFFF

            h, g, f, e, d, c, b, a = g, f, e, (d + t1) & 0xFFFFFFFF, c, b, a, (t1 + t2) & 0xFFFFFFFF

        h0 = (h0 + a) & 0xFFFFFFFF
        h1 = (h1 + b) & 0xFFFFFFFF
        h2 = (h2 + c) & 0xFFFFFFFF
        h3 = (h3 + d) & 0xFFFFFFFF
        h4 = (h4 + e) & 0xFFFFFFFF
        h5 = (h5 + f) & 0xFFFFFFFF
        h6 = (h6 + g) & 0xFFFFFFFF
        h7 = (h7 + h) & 0xFFFFFFFF

    digest = 0

    for i, v in enumerate([h0, h1, h2, h3, h4, h5, h6, h7]):
        digest = digest + (v << ((7 - i) * 32))

    return f"{digest:x}"


def sha512(message: bytes | str) -> str:
    if isinstance(message, str):
        message = message.encode("utf-8")

    h0, h1, h2, h3, h4, h5, h6, h7 = SHA512_H
    k = SHA512_K
    bit_size = SHA512_BITSIZE
    mask = SHA512_BITMASK

    for chunk in __chunk(__pad(message, 1024), 1024):
        w = [0] * 80

        for i in range(15, -1, -1):
            w[15 - i] = __isolate(chunk, 64, (i * 64))

        for i in range(16, 80):
            s0 = _rr(w[i - 15], 1, bit_size, mask) ^ _rr(w[i - 15], 8, bit_size, mask) ^ (w[i - 15] >> 7)
            s1 = _rr(w[i - 2], 19, bit_size, mask) ^ _rr(w[i - 2], 61, bit_size, mask) ^ (w[i - 2] >> 6)
            w[i] = (w[i - 16] + s0 + w[i - 7] + s1) & 0xFFFFFFFFFFFFFFFF

        a, b, c, d, e, f, g, h = h0, h1, h2, h3, h4, h5, h6, h7

        for i in range(80):
            ts1 = _rr(e, 14, bit_size, mask) ^ _rr(e, 18, bit_size, mask) ^ _rr(e, 41, bit_size, mask)  # noqa
            ch = (e & f) ^ ((~e) & g)
            t1 = (h + ts1 + ch + k[i] + w[i]) & 0xFFFFFFFFFFFFFFFF
            ts0 = _rr(a, 28, bit_size, mask) ^ _rr(a, 34, bit_size, mask) ^ _rr(a, 39, bit_size, mask)  # noqa
            ma = (a & b) ^ (a & c) ^ (b & c)
            t2 = (ts0 + ma) & 0xFFFFFFFFFFFFFFFF

            h, g, f, e, d, c, b, a = g, f, e, (d + t1) & 0xFFFFFFFFFFFFFFFF, c, b, a, (t1 + t2) & 0xFFFFFFFFFFFFFFFF

        h0 = (h0 + a) & 0xFFFFFFFFFFFFFFFF
        h1 = (h1 + b) & 0xFFFFFFFFFFFFFFFF
        h2 = (h2 + c) & 0xFFFFFFFFFFFFFFFF
        h3 = (h3 + d) & 0xFFFFFFFFFFFFFFFF
        h4 = (h4 + e) & 0xFFFFFFFFFFFFFFFF
        h5 = (h5 + f) & 0xFFFFFFFFFFFFFFFF
        h6 = (h6 + g) & 0xFFFFFFFFFFFFFFFF
        h7 = (h7 + h) & 0xFFFFFFFFFFFFFFFF

    digest = 0

    for i, v in enumerate([h0, h1, h2, h3, h4, h5, h6, h7]):
        digest = digest + (v << ((7 - i) * 64))

    return f"{digest:x}"
